...
short_usa_stoch = 'https://discord.com/api/webhooks/800242744630050867/e3c1qvdw_Q0wWnE8gc2w9zpnWSw-L2kqiYBpFmSgNOjDLAPSuMvQqHcU7XTFJHzSTzSS'
...
short_ftx_bb = 'https://discord.com/api/webhooks/800241400218124319/BjD_cQJM7e4AC5HTSfjhv-EEvWLEWO865ouH3vejyOL8q34BZzpXVL1ok5GKy0bgucoe'
...
short_binance_trap = 'https://discord.com/api/webhooks/800241202641240104/D0hrAUzkm5SllEccs0O0GBZQxmOKVOP2NYEWpjSvaRS_PdYUvEX3aAlug9eH7ANDhZLW'
...
def bb_drawer_usa(stock,islong,positivetrend,start_time = start, end_time = end):
        ...
        if len(df) >= 193:
            ...
            elif df['Close'][df.index[-1]] < df['LowerBand'][df.index[-1]] and positivetrend:
                if islong:
                    apdddddd = fplt.make_addplot(df['ema50'])
                    apddddddd = fplt.make_addplot(df['ema100'])
                    apdddddddd = fplt.make_addplot(df['ema200'])
            ...
            elif df['Close'][df.index[-1]] > df['UpperBand'][df.index[-1]] and not(positivetrend) :
                if islong:
                    apdddddd = fplt.make_addplot(df['ema50'])
                    apddddddd = fplt.make_addplot(df['ema100'])
                    apdddddddd = fplt.make_addplot(df['ema200'])
...
def osc_drawer_usa(stock,islong,positivetrend,start_time = start, end_time = end):
    df = yf.download(stock ,start= start_time, end = end_time)
    df['70'] = 70
    df['30'] = 30
    df['ema10'] = pd.Series.ewm(df['Close'], span=10, adjust=False).mean()
    ...
    if df["%K"][df.index[-1]] < 30 and islong and positivetrend :
        ...
        if islong:
            ...
            ema10 = fplt.make_addplot(df['ema10'])
            ema20 = fplt.make_addplot(df['ema20'])
            ema30 = fplt.make_addplot(df['ema30'])
            fig, axlist = fplt.plot(
                df,
                type="candle",
                title=stock,
                ylabel='Price($)',
                style='charles',
                addplot=[stoch1, stoch2, ema10, ema20,ema30,level70,level30], returnfig=True)
        ax1 = axlist[0]
        ax1_minor_yticks = ax1.get_yticks(True)
        ax1_major_yticks = ax1.get_yticks(False)
        ax1.set_yscale('log')
        ...
        at.insert({'ID': str(stock), 'Date': str(dt.datetime.date(dt.datetime.today())), 'Markets': 'Usa_stocks',
                  ...
                  typecast=True)
    elif df["%K"][df.index[-1]] < 30 and positivetrend :
        df = df.iloc[-180:]
        stoch1 = fplt.make_addplot(df['%K'], panel=1)
        stoch2 = fplt.make_addplot(df['%D'], panel=1)
        level70 = fplt.make_addplot(df['70'], panel=1, color='gray')
        ...
        at.insert({'ID': str(stock), 'Date': str(dt.datetime.date(dt.datetime.today())), 'Markets': 'Usa_stocks', 'Sell or Buy' : 'Sell', 'Trend' : 'long','Strategies' : 'stoch', 'entry': df['High'][df.index[-1]], 'close' : df['Low'][df.index[-1]] , 'Graph' : a},
            typecast=True)
    elif df["%K"][df.index[-1]] > 70 and not(positivetrend):
        df = df.iloc[-180:]
        stoch1 = fplt.make_addplot(df['%K'], panel=1)
        stoch2 = fplt.make_addplot(df['%D'], panel=1)
        level70 = fplt.make_addplot(df['70'], panel=1, color='gray')
        ...
        at.insert({'ID': str(stock), 'Date': str(dt.datetime.date(dt.datetime.today())), 'Markets': 'Usa_stocks', 'Sell or Buy' : 'Sell', 'Trend' : 'short', 'Strategies' : 'stoch', 'entry': df['High'][df.index[-1]], 'close' : df['Low'][df.index[-1]], 'Graph' : a},
            typecast=True)
def trap_drawer_usa(stock,islong,positivetrend,start_time = start, end_time = end):
    df = yf.download(stock ,start= start_time, end = end_time)
    df['ema10'] = pd.Series.ewm(df['Close'], span=10, adjust=False).mean()
    df['ema20'] = pd.Series.ewm(df['Close'], span=20, adjust=False).mean()
    df['ema30'] = pd.Series.ewm(df['Close'], span=30, adjust=False).mean()
    ...
    df['ema200'] = pd.Series.ewm(df['Close'], span=200, adjust=False).mean()
    df = df.iloc[-180:]
    if df['Low'][df.index[-1]] < df['Low'][df.index[-2]] and df['Low'][df.index[-1]] < df['Low'][df.index[-3]] and df['Close'][df.index[-1]] >= df['Open'][df.index[-1]] and islong and positivetrend:
        df = df.iloc[-180:]
        if islong:
            apdddddd = fplt.make_addplot(df['ema50'])
            apddddddd = fplt.make_addplot(df['ema100'])
    ...
    elif df['Low'][df.index[-1]] < df['Low'][df.index[-2]] and df['Low'][df.index[-1]] < df['Low'][df.index[-3]] and df['Close'][df.index[-1]] >= df['Open'][df.index[-1]] and positivetrend:
        df = df.iloc[-180:]
        if islong:
            apdddddd = fplt.make_addplot(df['ema50'])
            apddddddd = fplt.make_addplot(df['ema100'])
    ...
    elif df['High'][df.index[-1]] > df['High'][df.index[-2]] and df['High'][df.index[-1]] > df['High'][df.index[-3]] and df['Close'][df.index[-1]] <= df['Open'][df.index[-1]] and islong and not(positivetrend) :
        df = df.iloc[-180:]
        if islong:
            apdddddd = fplt.make_addplot(df['ema50'])
            apddddddd = fplt.make_addplot(df['ema100'])
...
def inside_drawer_usa(stock,islong,positivetrend,start_time = start, end_time = end):
    df = yf.download(stock ,start= start_time, end = end_time)
    df['ema10'] = pd.Series.ewm(df['Close'], span=10, adjust=False).mean()
    df['ema20'] = pd.Series.ewm(df['Close'], span=20, adjust=False).mean()
    df['ema30'] = pd.Series.ewm(df['Close'], span=30, adjust=False).mean()
    ...
    df['ema100'] = pd.Series.ewm(df['Close'], span=100, adjust=False).mean()
    df['ema200'] = pd.Series.ewm(df['Close'], span=200, adjust=False).mean()
    if df['High'][df.index[-1]] <= df['High'][df.index[-2]] and df['Low'][df.index[-1]] >= df['Low'][df.index[-2]] and islong and positivetrend:
        df = df.iloc[-180:]
        if islong:
            apdddddd = fplt.make_addplot(df['ema50'])
            apddddddd = fplt.make_addplot(df['ema100'])
    ...
    elif df['High'][df.index[-1]] <= df['High'][df.index[-2]] and df['Low'][df.index[-1]] >= df['Low'][df.index[-2]] and islong and not(positivetrend) :
        df = df.iloc[-180:]
        if islong:
            apdddddd = fplt.make_addplot(df['ema50'])
            apddddddd = fplt.make_addplot(df['ema100'])
...
def getftx_for_ema(stock):
    ...
    return df
def discorderr(webhookl,stock, rec, entry,img, close = 'is not provided'):
    webhook = DiscordWebhook(url= webhookl, username="FinanceBot")
    embed = DiscordEmbed(title=stock, description='Recommendation : ' + rec , color=242424)
    embed.add_embed_field(name='Entry at : ', value="{:10.7f}".format(entry))
    if close != 'is not provided':
        embed.add_embed_field(name='Close at ', value="{:10.7f}".format(close))
...
for i in long_term_positive:
    bb_drawer_usa(i,True, True)
    osc_drawer_usa(i, True, True)
    trap_drawer_usa(i,True, True)
    inside_drawer_usa(i, True, True)
for i in short_term_positive:
    bb_drawer_usa(i, False, True)
    osc_drawer_usa(i, False, True)
    trap_drawer_usa(i,False, True)
    inside_drawer_usa(i,False, True)
for i in long_term_negative:
    bb_drawer_usa(i,True, False)
    osc_drawer_usa(i,True, False)
    trap_drawer_usa(i,True, False)
    inside_drawer_usa(i, True, False)
for i in short_term_negative:
    bb_drawer_usa(i,False, False)
    osc_drawer_usa(i,False, False)
    trap_drawer_usa(i,False, False)
...
for i in range(len(bub)):
    if bub[i]['fields']['Markets'][0].lower() == 'usa_stocks':
        if bub[i]['fields']['Trend'][0].lower() == 'short':
            if bub[i]['fields']['Strategies'].lower() == 'stoch':
                if bub[i]['fields']['Sell or Buy'].lower() == 'buy':
                    discorderr(short_usa_stoch, bub[i]['fields']['ID'], bub[i]['fields']['Sell or Buy'],
                               bub[i]['fields']['entry'], bub[i]['fields']['Graph'], bub[i]['fields']['close'])
                    time.sleep(2)
                ...
                if bub[i]['fields']['Sell or Buy'].lower() == 'buy':
                    discorderr(short_usa_bb, bub[i]['fields']['ID'], bub[i]['fields']['Sell or Buy'],
                               bub[i]['fields']['entry'], bub[i]['fields']['Graph'])
                    time.sleep(2)
                elif  bub[i]['fields']['Sell or Buy'].lower() == 'sell':
                    discorderr(short_usa_bb, bub[i]['fields']['ID'], bub[i]['fields']['Sell or Buy'],
                               bub[i]['fields']['entry'], bub[i]['fields']['Graph'])
                ...
                if bub[i]['fields']['Sell or Buy'].lower() == 'buy':
                    discorderr(short_usa_trap, bub[i]['fields']['ID'], bub[i]['fields']['Sell or Buy'],
                               bub[i]['fields']['entry'], bub[i]['fields']['Graph'], bub[i]['fields']['close'])
                    time.sleep(2)
                elif bub[i]['fields']['Sell or Buy'].lower() == 'sell':
                    discorderr(short_usa_trap, bub[i]['fields']['ID'], bub[i]['fields']['Sell or Buy'],
                ...
                if bub[i]['fields']['Sell or Buy'].lower() == 'buy':
                    discorderr(short_usa_inside, bub[i]['fields']['ID'], bub[i]['fields']['Sell or Buy'],
                               bub[i]['fields']['entry'], bub[i]['fields']['Graph'], bub[i]['fields']['close'])
                    time.sleep(2)
                elif bub[i]['fields']['Sell or Buy'].lower() == 'sell':
                    discorderr(short_usa_inside, bub[i]['fields']['ID'], bub[i]['fields']['Sell or Buy'],
            ...
            if bub[i]['fields']['Strategies'].lower() == 'stoch':
                if bub[i]['fields']['Sell or Buy'].lower() == 'buy':
                    discorderr(long_usa_stoch, bub[i]['fields']['ID'], bub[i]['fields']['Sell or Buy'],
                               bub[i]['fields']['entry'], bub[i]['fields']['Graph'], bub[i]['fields']['close'])
                    time.sleep(2)
                elif  bub[i]['fields']['Sell or Buy'].lower() == 'sell':
                    discorderr(long_usa_stoch, bub[i]['fields']['ID'], bub[i]['fields']['Sell or Buy'],
                ...
                if bub[i]['fields']['Sell or Buy'].lower() == 'buy':
                    discorderr(long_usa_bb, bub[i]['fields']['ID'], bub[i]['fields']['Sell or Buy'],
                               bub[i]['fields']['entry'], bub[i]['fields']['Graph'])
                    time.sleep(2)
                elif  bub[i]['fields']['Sell or Buy'].lower() == 'sell':
                    discorderr(long_usa_bb, bub[i]['fields']['ID'], bub[i]['fields']['Sell or Buy'],
                ...
                if bub[i]['fields']['Sell or Buy'].lower() == 'buy':
                    discorderr(long_usa_trap, bub[i]['fields']['ID'], bub[i]['fields']['Sell or Buy'],
                               bub[i]['fields']['entry'], bub[i]['fields']['Graph'], bub[i]['fields']['close'])
                    time.sleep(2)
                elif bub[i]['fields']['Sell or Buy'].lower() == 'sell':
                    discorderr(long_usa_trap, bub[i]['fields']['ID'], bub[i]['fields']['Sell or Buy'],
                ...
                if bub[i]['fields']['Sell or Buy'].lower() == 'buy':
                    discorderr(long_usa_inside, bub[i]['fields']['ID'], bub[i]['fields']['Sell or Buy'],
                               bub[i]['fields']['entry'], bub[i]['fields']['Graph'], bub[i]['fields']['close'])
                    time.sleep(2)
                elif bub[i]['fields']['Sell or Buy'].lower() == 'sell':
                    discorderr(long_usa_inside, bub[i]['fields']['ID'], bub[i]['fields']['Sell or Buy'],
        ...
        if bub[i]['fields']['Trend'][0].lower() == 'short':
            if bub[i]['fields']['Strategies'].lower() == 'stoch':
                if bub[i]['fields']['Sell or Buy'].lower() == 'buy':
                    discorderr(short_br_stoch, bub[i]['fields']['ID'], bub[i]['fields']['Sell or Buy'],
                               bub[i]['fields']['entry'], bub[i]['fields']['Graph'], bub[i]['fields']['close'])
                    time.sleep(2)
                elif  bub[i]['fields']['Sell or Buy'].lower() == 'sell':
                    discorderr(short_br_stoch, bub[i]['fields']['ID'], bub[i]['fields']['Sell or Buy'],
                ...
                if bub[i]['fields']['Sell or Buy'].lower() == 'buy':
                    discorderr(short_br_bb,bub[i]['fields']['ID'], bub[i]['fields']['Sell or Buy'],
                               bub[i]['fields']['entry'], bub[i]['fields']['Graph'])
                    time.sleep(2)
                elif  bub[i]['fields']['Sell or Buy'].lower() == 'sell':
                    discorderr(short_br_bb, bub[i]['fields']['ID'], bub[i]['fields']['Sell or Buy'],
                ...
                if bub[i]['fields']['Sell or Buy'].lower() == 'buy':
                    discorderr(short_br_trap, bub[i]['fields']['ID'], bub[i]['fields']['Sell or Buy'],
                               bub[i]['fields']['entry'], bub[i]['fields']['Graph'], bub[i]['fields']['close'])
                    time.sleep(2)
                elif bub[i]['fields']['Sell or Buy'].lower() == 'sell':
                    discorderr(short_br_trap, bub[i]['fields']['ID'], bub[i]['fields']['Sell or Buy'],
                ...
                if bub[i]['fields']['Sell or Buy'].lower() == 'buy':
                    discorderr(short_br_inside, bub[i]['fields']['ID'], bub[i]['fields']['Sell or Buy'],
                               bub[i]['fields']['entry'], bub[i]['fields']['Graph'], bub[i]['fields']['close'])
                    time.sleep(2)
                elif bub[i]['fields']['Sell or Buy'].lower() == 'sell':
                    discorderr(short_br_inside, bub[i]['fields']['ID'], bub[i]['fields']['Sell or Buy'],
            ...
            if bub[i]['fields']['Strategies'].lower() == 'stoch':
                if bub[i]['fields']['Sell or Buy'].lower() == 'buy':
                    discorderr(long_br_stoch, bub[i]['fields']['ID'], bub[i]['fields']['Sell or Buy'],
                               bub[i]['fields']['entry'], bub[i]['fields']['Graph'], bub[i]['fields']['close'])
                    time.sleep(2)
                elif  bub[i]['fields']['Sell or Buy'].lower() == 'sell':
                    discorderr(long_br_stoch, bub[i]['fields']['ID'], bub[i]['fields']['Sell or Buy'],
                ...
                if bub[i]['fields']['Sell or Buy'].lower() == 'buy':
                    discorderr(long_br_bb, bub[i]['fields']['ID'], bub[i]['fields']['Sell or Buy'],
                               bub[i]['fields']['entry'], bub[i]['fields']['Graph'])
                    time.sleep(2)
                elif  bub[i]['fields']['Sell or Buy'].lower() == 'sell':
                    discorderr(long_br_bb, bub[i]['fields']['ID'], bub[i]['fields']['Sell or Buy'],
                ...
                if bub[i]['fields']['Sell or Buy'].lower() == 'buy':
                    discorderr(long_br_trap, bub[i]['fields']['ID'], bub[i]['fields']['Sell or Buy'],
                               bub[i]['fields']['entry'], bub[i]['fields']['Graph'], bub[i]['fields']['close'])
                    time.sleep(2)
                elif bub[i]['fields']['Sell or Buy'].lower() == 'sell':
                    discorderr(long_br_trap, bub[i]['fields']['ID'], bub[i]['fields']['Sell or Buy'],
                ...
                if bub[i]['fields']['Sell or Buy'].lower() == 'buy':
                    discorderr(long_br_inside, bub[i]['fields']['ID'], bub[i]['fields']['Sell or Buy'],
                               bub[i]['fields']['entry'], bub[i]['fields']['Graph'], bub[i]['fields']['close'])
                    time.sleep(2)
                elif bub[i]['fields']['Sell or Buy'].lower() == 'sell':
                    discorderr(long_br_inside, bub[i]['fields']['ID'], bub[i]['fields']['Sell or Buy'],
        ...
        if bub[i]['fields']['Trend'][0].lower() == 'short':
            if bub[i]['fields']['Strategies'].lower() == 'stoch':
                if bub[i]['fields']['Sell or Buy'].lower() == 'buy':
                    discorderr(short_binance_stoch, bub[i]['fields']['ID'], bub[i]['fields']['Sell or Buy'],
                               bub[i]['fields']['entry'], bub[i]['fields']['Graph'], bub[i]['fields']['close'])
                    time.sleep(2)
                elif  bub[i]['fields']['Sell or Buy'].lower() == 'sell':
                    discorderr(short_binance_stoch, bub[i]['fields']['ID'], bub[i]['fields']['Sell or Buy'],
                ...
                if bub[i]['fields']['Sell or Buy'].lower() == 'buy':
                    discorderr(short_binance_bb, bub[i]['fields']['ID'], bub[i]['fields']['Sell or Buy'],
                               bub[i]['fields']['entry'], bub[i]['fields']['Graph'])
                    time.sleep(2)
                elif  bub[i]['fields']['Sell or Buy'].lower() == 'sell':
                    discorderr(short_binance_bb, bub[i]['fields']['ID'], bub[i]['fields']['Sell or Buy'],
                ...
                if bub[i]['fields']['Sell or Buy'].lower() == 'buy':
                    discorderr(short_binance_trap, bub[i]['fields']['ID'], bub[i]['fields']['Sell or Buy'],
                               bub[i]['fields']['entry'], bub[i]['fields']['Graph'], bub[i]['fields']['close'])
                    time.sleep(2)
                elif bub[i]['fields']['Sell or Buy'].lower() == 'sell':
                    discorderr(short_binance_trap, bub[i]['fields']['ID'], bub[i]['fields']['Sell or Buy'],
                ...
                if bub[i]['fields']['Sell or Buy'].lower() == 'buy':
                    discorderr(short_binance_inside, bub[i]['fields']['ID'], bub[i]['fields']['Sell or Buy'],
                               bub[i]['fields']['entry'], bub[i]['fields']['Graph'], bub[i]['fields']['close'])
                    time.sleep(2)
                elif bub[i]['fields']['Sell or Buy'].lower() == 'sell':
                    discorderr(short_binance_inside, bub[i]['fields']['ID'], bub[i]['fields']['Sell or Buy'],
            ...
            if bub[i]['fields']['Strategies'].lower() == 'stoch':
                if bub[i]['fields']['Sell or Buy'].lower() == 'buy':
                    discorderr(long_binance_stoch, bub[i]['fields']['ID'], bub[i]['fields']['Sell or Buy'],
                               bub[i]['fields']['entry'], bub[i]['fields']['Graph'], bub[i]['fields']['close'])
                    time.sleep(2)
                elif  bub[i]['fields']['Sell or Buy'].lower() == 'sell':
                    discorderr(long_binance_stoch, bub[i]['fields']['ID'], bub[i]['fields']['Sell or Buy'],
                ...
                if bub[i]['fields']['Sell or Buy'].lower() == 'buy':
                    discorderr(long_binance_bb, bub[i]['fields']['ID'], bub[i]['fields']['Sell or Buy'],
                               bub[i]['fields']['entry'], bub[i]['fields']['Graph'])
                    time.sleep(2)
                elif  bub[i]['fields']['Sell or Buy'].lower() == 'sell':
                    discorderr(long_binance_bb, bub[i]['fields']['ID'], bub[i]['fields']['Sell or Buy'],
                ...
                if bub[i]['fields']['Sell or Buy'].lower() == 'buy':
                    discorderr(long_binance_trap, bub[i]['fields']['ID'], bub[i]['fields']['Sell or Buy'],
                               bub[i]['fields']['entry'], bub[i]['fields']['Graph'], bub[i]['fields']['close'])
                    time.sleep(2)
                elif bub[i]['fields']['Sell or Buy'].lower() == 'sell':
                    discorderr(long_binance_trap, bub[i]['fields']['ID'], bub[i]['fields']['Sell or Buy'],
                ...
                if bub[i]['fields']['Sell or Buy'].lower() == 'buy':
                    discorderr(long_binance_inside, bub[i]['fields']['ID'], bub[i]['fields']['Sell or Buy'],
                               bub[i]['fields']['entry'], bub[i]['fields']['Graph'], bub[i]['fields']['close'])
                    time.sleep(2)
                elif bub[i]['fields']['Sell or Buy'].lower() == 'sell':
                    discorderr(long_binance_inside, bub[i]['fields']['ID'], bub[i]['fields']['Sell or Buy'],
        ...
        if bub[i]['fields']['Trend'][0].lower() == 'short':
            if bub[i]['fields']['Strategies'].lower() == 'stoch':
                if bub[i]['fields']['Sell or Buy'].lower() == 'buy':
                    discorderr(short_ftx_stoch, bub[i]['fields']['ID'], bub[i]['fields']['Sell or Buy'],
                               bub[i]['fields']['entry'], bub[i]['fields']['Graph'], bub[i]['fields']['close'])
                    time.sleep(2)
                elif  bub[i]['fields']['Sell or Buy'].lower() == 'sell':
                    discorderr(short_ftx_stoch, bub[i]['fields']['ID'], bub[i]['fields']['Sell or Buy'],
                ...
                if bub[i]['fields']['Sell or Buy'].lower() == 'buy':
                    discorderr(short_ftx_bb, bub[i]['fields']['ID'], bub[i]['fields']['Sell or Buy'],
                               bub[i]['fields']['entry'], bub[i]['fields']['Graph'])
                    time.sleep(2)
                elif  bub[i]['fields']['Sell or Buy'].lower() == 'sell':
                    discorderr(short_ftx_bb, bub[i]['fields']['ID'], bub[i]['fields']['Sell or Buy'],
                ...
                if bub[i]['fields']['Sell or Buy'].lower() == 'buy':
                    discorderr(short_ftx_trap, bub[i]['fields']['ID'], bub[i]['fields']['Sell or Buy'],
                               bub[i]['fields']['entry'], bub[i]['fields']['Graph'], bub[i]['fields']['close'])
                    time.sleep(2)
                elif bub[i]['fields']['Sell or Buy'].lower() == 'sell':
                    discorderr(short_ftx_trap, bub[i]['fields']['ID'], bub[i]['fields']['Sell or Buy'],
                ...
                if bub[i]['fields']['Sell or Buy'].lower() == 'buy':
                    discorderr(short_ftx_inside, bub[i]['fields']['ID'], bub[i]['fields']['Sell or Buy'],
                               bub[i]['fields']['entry'], bub[i]['fields']['Graph'], bub[i]['fields']['close'])
                    time.sleep(2)
                elif bub[i]['fields']['Sell or Buy'].lower() == 'sell':
                    discorderr(short_ftx_inside, bub[i]['fields']['ID'], bub[i]['fields']['Sell or Buy'],
            ...
            if bub[i]['fields']['Strategies'].lower() == 'stoch':
                if bub[i]['fields']['Sell or Buy'].lower() == 'buy':
                    discorderr(long_ftx_stoch, bub[i]['fields']['ID'], bub[i]['fields']['Sell or Buy'],
                               bub[i]['fields']['entry'], bub[i]['fields']['Graph'], bub[i]['fields']['close'])
                    time.sleep(2)
                elif  bub[i]['fields']['Sell or Buy'].lower() == 'sell':
                    discorderr(long_ftx_stoch, bub[i]['fields']['ID'], bub[i]['fields']['Sell or Buy'],
                ...
                if bub[i]['fields']['Sell or Buy'].lower() == 'buy':
                    discorderr(long_ftx_bb, bub[i]['fields']['ID'], bub[i]['fields']['Sell or Buy'],
                               bub[i]['fields']['entry'], bub[i]['fields']['Graph'])
                    time.sleep(2)
                elif  bub[i]['fields']['Sell or Buy'].lower() == 'sell':
                    discorderr(long_ftx_bb, bub[i]['fields']['ID'], bub[i]['fields']['Sell or Buy'],
                ...
                if bub[i]['fields']['Sell or Buy'].lower() == 'buy':
                    discorderr(long_ftx_trap, bub[i]['fields']['ID'], bub[i]['fields']['Sell or Buy'],
                               bub[i]['fields']['entry'], bub[i]['fields']['Graph'], bub[i]['fields']['close'])
                    time.sleep(2)
                elif bub[i]['fields']['Sell or Buy'].lower() == 'sell':
                    discorderr(long_ftx_trap, bub[i]['fields']['ID'], bub[i]['fields']['Sell or Buy'],
                ...
                if bub[i]['fields']['Sell or Buy'].lower() == 'buy':
                    discorderr(long_ftx_inside, bub[i]['fields']['ID'], bub[i]['fields']['Sell or Buy'],
                               bub[i]['fields']['entry'], bub[i]['fields']['Graph'], bub[i]['fields']['close'])
                    time.sleep(2)
                elif bub[i]['fields']['Sell or Buy'].lower() == 'sell':
                    discorderr(long_ftx_inside, bub[i]['fields']['ID'], bub[i]['fields']['Sell or Buy'],
